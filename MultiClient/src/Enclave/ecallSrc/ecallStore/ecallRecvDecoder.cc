 ...
 */
uint8_t *EcallRecvDecoder::xd3_decode(uint8_t *in, size_t in_size, uint8_t *ref, size_t ref_size, size_t *res_size) // 更改函数
{
    ...
    size_t sz;
    uint32_t sz32;
    uint32_t in_size32 = in_size;
    uint32_t ref_size32 = ref_size;
    // xd3_decode_memory(in, in_size, ref, ref_size, buffer, &sz, max_buffer_size, 0);
    int resedelta = EDeltaDecode(in, in_size32, ref, ref_size32, buffer, &sz32);
    uint8_t *res;
    sz = sz32;
            ...
            for (size_t idx = 0; idx < sgxClient->_enclaveRecipeBuffer.size(); idx++)
            {
                ...
                if (deltaflag == DELTA)
                {
                    ...
                    if (decompressedSize > 0)
                    {
                        ...
                        recchunk = xd3_decode((uint8_t *)&decompressedChunk, chunkSize, (uint8_t *)&lz4decompressbaseChunk, decompressedSize, &recchunk_size);
                    }
                    else
                    {
                        // base chunk can decompress, use it as basechunk
                        recchunk = xd3_decode((uint8_t *)&decompressedChunk, chunkSize, (uint8_t *)&decompressbaseChunk, baseChunkLength, &recchunk_size);
                    }
                    // delta_num++;
