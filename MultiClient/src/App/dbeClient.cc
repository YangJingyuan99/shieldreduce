...
string myName = "ShieldReduceClient";
string logFileName = "client-log";
ofstream logFile;

void Usage() {
    fprintf(stderr, "./ShieldReduceClient -t [u/d/o] -i [inputFile path].\n"
    "-t: operation ([u/d/a]):\n"
    "\tu: upload\n"
    "\td: download\n"
    "\to: upload with offline opeartion\n");
    return ;
}
...
int main(int argc, char* argv[]) {
    ...
    if (!tool::FileExist(logFileName)) {
        ...
        logFile << "input file, " << "opt, "
            ...
            << "total time (s), " << "speed (MiB/s), " <<"chunking time, "<<"total filesize, "<<"total chunkingtime"<<endl;
    } else {
        // the log file exists
        ...
        switch (option) {
            case 't':
                if (strcmp("u", optarg) == 0) {
                    optType = UPLOAD_OPT;
                    tool::Logging(myName.c_str(), "upload\n");
                    break;
                } else if (strcmp("o", optarg) == 0) {
                    optType = OFFLINE_OPT;
                    tool::Logging(myName.c_str(), "offline\n");
                    break;
    ...
    switch (optType) {
        case UPLOAD_OPT:
        case OFFLINE_OPT:  { // for upload operation
            ...
            dataSenderObj->UploadLogin(config.GetLocalSecret(), fileNameHash, optType);
            ...
            totalsize += chunkerObj->_recipe.recipeHead.fileSize;
            totalchunkTime += chunkerObj->totalTime_- chunkerObj->insertTime_;
            logFile << inputFile << ", upload, "
                << chunkerObj->_recipe.recipeHead.fileSize << ", "
                << chunkerObj->_recipe.recipeHead.totalChunkNum << ", "
                << to_string(totalTime) << ", "
                << to_string(speed) <<","<<to_string(totalchunkTime*1000/totalsize*1024*1024)<<","<< to_string(totalsize) <<","<<to_string(totalchunkTime) <<endl;
            delete chunkerObj;
